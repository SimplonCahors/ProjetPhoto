...
class themedoSC_Separator {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_separator-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_separator-shortcode-icon-wrapper', array( $this, 'icon_wrapper_attr' ) );
		add_filter( 'avalon_td_attr_separator-shortcode-icon', array( $this, 'icon_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$defaults = themedoCore_Plugin::set_shortcode_defaults(
			array(
				...
				'alignment'			=> 'center',
				'bottom_margin'		=> '',
				'border_size'		=> $avalon_td_option['separator_border_size'],
				'icon'				=> '',
				'icon_circle'		=> $avalon_td_option['separator_circle'],
				'icon_circle_color'	=> '',
				'sep_color'			=> $avalon_td_option['sep_color'],
				'style_type'		=> 'none',
			   	'top_margin'		=> '',
			   	'width'				=> '',
			   	'bottom'			=> '',	//deprecated
		...
		if ( $icon && 
			...
			$icon_insert = sprintf( '<span %s><i %s></i></span>', themedoCore_Plugin::attributes( 'separator-shortcode-icon-wrapper' ), themedoCore_Plugin::attributes( 'separator-shortcode-icon' ) );
		} else {
			$icon_insert = '';
		}

		$html = sprintf( '<div %s></div><div %s>%s</div>', themedoCore_Plugin::attributes( 'themedo-sep-clear' ), themedoCore_Plugin::attributes( 'separator-shortcode' ), $icon_insert );

		if ( self::$args['alignment'] == 'right' ) {
			$html .= sprintf( '<div %s></div>', themedoCore_Plugin::attributes( 'themedo-sep-clear' ) );
		}
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'themedo-separator' );
		$attr['style'] = '';


		if( ! self::$args['width'] || self::$args['width'] == '100%' ) {
			$attr['class'] .= ' themedo-full-width-sep';
		}
