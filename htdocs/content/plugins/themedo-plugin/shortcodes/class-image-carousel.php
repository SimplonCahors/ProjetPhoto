...
class themedoSC_ImageCarousel {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_image-carousel-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_image-carousel-shortcode-carousel', array( $this, 'carousel_attr' ) );
		add_filter( 'avalon_td_attr_image-carousel-shortcode-slide-link', array( $this, 'slide_link_attr' ) );
		add_filter( 'avalon_td_attr_themedo-image-wrapper', array( $this, 'image_wrapper' ) );
		//add_filter( 'avalon_td_attr_themedo-nav-prev', array( $this, 'avalon_td_nav_prev' ) );
		//add_filter( 'avalon_td_attr_themedo-nav-next', array( $this, 'avalon_td_nav_next' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$html = sprintf( '<div %s>', themedoCore_Plugin::attributes( 'image-carousel-shortcode' ) );
			$html .= sprintf( '<div %s>', themedoCore_Plugin::attributes( 'image-carousel-shortcode-carousel' ) );
	...
	function attr() {
		...
		$attr['class'] = 'themedo-image-carousel themedo-image-carousel-' . self::$parent_args['picture_size'];		
		...
		if( self::$parent_args['border'] == 'yes' ) {
		  $attr['class'] .= ' themedo-carousel-border';
		}		
	...
	function render_child( $args, $content = '') {
		...
		if( self::$parent_args['mouse_scroll'] == 'no' &&
			...
			$output = sprintf( '<a %s>%s</a>', themedoCore_Plugin::attributes( 'image-carousel-shortcode-slide-link' ), $output );
		}

		$html = sprintf( '<li %s><div %s><div %s>%s</div></div></li>', themedoCore_Plugin::attributes( 'themedo-carousel-item'), themedoCore_Plugin::attributes( 'themedo-carousel-item-wrapper'), 
						 themedoCore_Plugin::attributes( 'themedo-image-wrapper' ), $output );
		return $html;
	...
	function slide_link_attr() {
		...
		if( self::$parent_args['lightbox'] == 'yes' ) {
			...
			$image_id = themedoCore_Plugin::get_attachment_id_from_url( self::$child_args['image'] );
	...
	function image_wrapper() {
		...
		$attr['class'] = 'themedo-image-wrapper';
	...
	function avalon_td_nav_prev() {

		$attr = array();

		$attr['class'] = 'themedo-nav-prev themedo-icon-left';

		return $attr;

	}

	function avalon_td_nav_next() {

		$attr = array();

		$attr['class'] = 'themedo-nav-next  themedo-icon-right';
