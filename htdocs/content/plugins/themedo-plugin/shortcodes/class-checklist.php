...
class themedoSC_Checklist {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_checklist-shortcode', array( $this, 'attr' ) );
		add_shortcode( 'checklist', array( $this, 'render_parent' ) );

		add_filter( 'avalon_td_attr_checklist-shortcode-li-item', array( $this, 'li_attr' ) );
		add_filter( 'avalon_td_attr_checklist-shortcode-span', array( $this, 'span_attr' ) );
		add_filter( 'avalon_td_attr_checklist-shortcode-icon', array( $this, 'icon_attr' ) );
		add_filter( 'avalon_td_attr_checklist-shortcode-item-content', array( $this, 'item_content_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$defaults = themedoCore_Plugin::set_shortcode_defaults(
			array(
				'class'			=> '',
				'id'			=> '',
				'circle'		=> strtolower( $avalon_td_option['checklist_circle'] ),
				'circlecolor'	=> $avalon_td_option['checklist_circle_color'],
				'icon'			=> 'fa-check',
				'iconcolor'		=> $avalon_td_option['checklist_icons_color'],
				'size'			=> '13px',
			), $args
		);
		...
		$html = sprintf( '<ul %s>%s</ul>', themedoCore_Plugin::attributes( 'checklist-shortcode' ), do_shortcode( $content ) );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'themedo-checklist themedo-checklist-%s', $this->checklist_counter );
	...
	function render_child( $args, $content = '') {
		...
		$html =  sprintf( '<li %s><span %s><i %s></i></span><div %s>%s</div></li>', themedoCore_Plugin::attributes( 'checklist-shortcode-li-item' ), themedoCore_Plugin::attributes( 'checklist-shortcode-span' ),
						  themedoCore_Plugin::attributes( 'checklist-shortcode-icon' ), themedoCore_Plugin::attributes( 'checklist-shortcode-item-content' ), do_shortcode( $content ) );
	...
	function item_content_attr() {
		...
		$attr['class'] = 'themedo-li-item-content';
	...
	function icon_attr() {
		...
		if( ! self::$child_args['icon'] ) {
			$icon = themedoCore_Plugin::font_awesome_name_handler( self::$parent_args['icon'] );
		} else {
			$icon = themedoCore_Plugin::font_awesome_name_handler( self::$child_args['icon'] );
		}
		...
		$attr['class'] = sprintf( 'themedo-li-icon fa %s', $icon );
