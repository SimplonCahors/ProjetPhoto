...
class themedoSC_themedoSlider {
	...
	public function __construct() {
		...
		add_shortcode( 'themedoslider', array( $this, 'render_parent' ) );

		add_filter( 'avalon_td_attr_themedo-slider-wrapper', array( $this, 'wrapper_attr' ) );
		add_filter( 'avalon_td_attr_themedo-slider-container', array( $this, 'container_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		if ( $query->have_posts() ) {
		?>
			<div <?php echo themedoCore_Plugin::attributes( 'themedo-slider-wrapper' ); ?>>
				<div class="themedo-slider-loading"><?php _e( 'Loading...', 'themedo-core' ); ?></div>
				<div <?php echo themedoCore_Plugin::attributes( 'themedo-slider-container' ); ?>>
					<ul class="slides">
						<?php
						while( $query->have_posts() ): $query->the_post();
							...
							if( isset ( $metadata['pyre_video_bg_color'][0] ) && $metadata['pyre_video_bg_color'][0] ) {
								$video_bg_color_hex = themedoCore_Plugin::hex2rgb( $metadata['pyre_video_bg_color'][0]  );
	...
	function wrapper_attr() {
		...
		$attr['class'] = 'themedo-slider-container';
