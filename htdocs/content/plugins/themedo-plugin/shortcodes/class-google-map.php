...
class themedoSC_GoogleMap {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_google-map-shortcode', array( $this, 'attr' ) );
	...
	function render( $args, $content = '' ) {
		...
		if( $address ) {
			...
			if( $map_style == 'theme' ) {
				...
				$animation = 'yes';
				$infobox = 'custom';
				$infobox_background_color = themedoCore_Plugin::hex2rgb( $avalon_td_option['primary_color'] );
				$infobox_background_color = 'rgba(' . $infobox_background_color[0] . ', ' . $infobox_background_color[1] . ', ' . $infobox_background_color[2] . ', 0.8)';
				$overlay_color = $avalon_td_option['primary_color'];
				$brightness_level = themedoCore_Plugin::calc_color_brightness( $avalon_td_option['primary_color'] );
			...
			if( $icon == 'theme' && $map_style == 'custom' ) {
				for( $i = 0; $i < $num_of_addresses; $i++ ) {
					$icon_array[$i] = plugins_url( 'images/avalon_td_map_marker.png', dirname( __FILE__ ) );				
				}
			}			
			...
			$cached_addresses = get_option( 'avalon_td_map_addresses' );
			...
			$map_id = uniqid( 'avalon_td_map_' ); // generate a unique ID for this map
			$this->map_id = $map_id;
			...
			<script type="text/javascript">
				...
				var markers = [];
				var counter = 0;
				function avalon_td_run_map_<?php echo $map_id ; ?>() {
					jQuery('#<?php echo $map_id ; ?>').avalon_td_maps({
						...
						overlay_color_hsl: <?php echo json_encode( themedoCore_Plugin::rgb2hsl( $overlay_color ) ); ?>,
				...
				google.maps.event.addDomListener(window, 'load', avalon_td_run_map_<?php echo $map_id ; ?>);
			</script>
			<?php
			if( $defaults['id'] ) {
				$html = ob_get_clean() . sprintf( '<div id="%s"><div %s></div></div>', $defaults['id'], themedoCore_Plugin::attributes( 'google-map-shortcode' ) );
			} else {
				$html = ob_get_clean() . sprintf( '<div %s></div>', themedoCore_Plugin::attributes( 'google-map-shortcode' ) );
			}
	...
	function attr() {
		...
		$attr['class'] = 'shortcode-map themedo-google-map';
	...
	function get_coordinates( $address, $force_refresh = false ) {
		...
		$key = $avalon_td_option['google_console_api_key'];
		...
		if ( $force_refresh || 
			...
			if ( $response['response']['code'] == 200 ) {
				...
				if ( $data->status === 'OK' ) {
				  	...
				  	return __( 'No location found for the entered address.', 'themedo-core' );
				} elseif( $data->status === 'INVALID_REQUEST' ) {
				   	return __( 'Invalid request. Did you enter an address?', 'themedo-core' );
				} else {
					return __( 'Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'themedo-core' );
				}

			} else {
			 	return __( 'Unable to contact Google API service.', 'themedo-core' );
			}
