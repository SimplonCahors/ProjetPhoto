...
class themedoSC_Blog {
	...
	public function __construct() {
		...
		// containers
		add_action( 'avalon_td_blog_shortcode_before_loop', array( $this, 'before_loop' ) );
		add_action( 'avalon_td_blog_shortcode_before_loop_timeline', array( $this, 'before_loop_timeline' ) );
		add_action( 'avalon_td_blog_shortcode_after_loop', array( $this, 'after_loop' ) );

		// post / loop basic structure
		add_action( 'avalon_td_blog_shortcode_loop_header', array( $this, 'loop_header' ) );
		add_action( 'avalon_td_blog_shortcode_loop_footer', array( $this, 'loop_footer' ) );
		add_action( 'avalon_td_blog_shortcode_loop_content', array( $this, 'loop_content' ) );
		add_action( 'avalon_td_blog_shortcode_loop_content', array( $this, 'page_links' ) );
		add_action( 'avalon_td_blog_shortcode_loop', array( $this, 'loop' ) );

		// special blog layout structure
		add_action( 'avalon_td_blog_shortcode_wrap_loop_open', array( $this, 'wrap_loop_open' ) );
		add_action( 'avalon_td_blog_shortcode_wrap_loop_close', array( $this, 'wrap_loop_close' ) );
		add_action( 'avalon_td_blog_shortcode_date_and_format', array( $this, 'add_date_box' ) );
		add_action( 'avalon_td_blog_shortcode_date_and_format', array( $this, 'add_format_box' ) );
		add_action( 'avalon_td_blog_shortcode_timeline_date', array( $this, 'timeline_date' ) );

		// element attributes
		add_filter( 'avalon_td_attr_blog-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_blog-shortcode-posts-container', array( $this, 'posts_container_attr' ) );
		add_filter( 'avalon_td_attr_blog-shortcode-loop', array( $this, 'loop_attr' ) );
		add_filter( 'avalon_td_attr_blog-shortcode-post-title', array( $this, 'post_title_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$avalon_td_query = new WP_Query( $defaults );	

		$this->query = $avalon_td_query;


		//prepare needed wrapping containers
		$html = '';

		$html .= sprintf( '<div %s>', themedoCore_Plugin::attributes( 'blog-shortcode' ) );
		
		if ( self::$args['layout'] == 'grid' &&
			 self::$args['blog_grid_column_spacing'] 
		) {
			$html .= sprintf( '<style type="text/css">.themedo-blog-shortcode-%s .themedo-blog-layout-grid .themedo-post-grid{padding:%spx;}.themedo-blog-shortcode-%s .themedo-posts-container{margin-left: -%spx !important; margin-right:-%spx !important;}</style>', $this->blog_sc_counter, $defaults['blog_grid_column_spacing'] / 2, $this->blog_sc_counter, $defaults['blog_grid_column_spacing'] / 2, $defaults['blog_grid_column_spacing'] / 2 );
		}

		$html .= sprintf( '<div %s>', themedoCore_Plugin::attributes( 'blog-shortcode-posts-container' ) );

		ob_start();
		do_action( 'avalon_td_blog_shortcode_wrap_loop_open' );
		...
		//do the loop
		if ( $avalon_td_query->have_posts() ) : while ( $avalon_td_query->have_posts() ) : $avalon_td_query->the_post();
			...
			if ( self::$args['layout'] == 'timeline' ) {
				...
				ob_start();
				do_action( 'avalon_td_blog_shortcode_timeline_date', $date_params );
			...
			ob_start();
			do_action( 'avalon_td_blog_shortcode_before_loop' );
			...
			if ( self::$args['layout'] == 'grid' || 
				...
				$html .= sprintf( '<div %s>', themedoCore_Plugin::attributes( 'themedo-post-wrapper' ) );
			}
			...
			ob_start();
			do_action( 'avalon_td_blog_shortcode_loop_header' );

			do_action( 'avalon_td_blog_shortcode_loop_content' );

			do_action( 'avalon_td_blog_shortcode_loop_footer' );
			
			do_action( 'avalon_td_blog_shortcode_after_loop' );
		...
		ob_start();
		do_action( 'avalon_td_blog_shortcode_wrap_loop_close' );
		...
		if ( self::$args['paging'] ) {
			ob_start();
			avalon_td_pagination( $this->query->max_num_pages, $range = 2, $this->query );
		...
		if ( self::$args['load_more'] && self::$args['posts_per_page'] != -1 ) {
			$html .= sprintf( '<div class="themedo-load-more-button themedo-clearfix">%s</div>', apply_filters( 'avalon_td_load_more_pots_name', __( 'Load More Posts', 'themedo-core' ) ) );
		}		
	...
	function attr() {
		...
		$blog_layout = '';
		if( self::$args['layout'] == 'timeline' ) {
			$blog_layout = 'themedo-blog-layout-timeline-wrapper';			
		} elseif ( self::$args['layout'] == 'grid' ) {
			$blog_layout = 'themedo-blog-layout-grid-wrapper';	
		} else {
			$blog_layout = sprintf( 'themedo-blog-layout-%s', self::$args['layout'] );
		}

		$attr['class'] = sprintf( 'themedo-blog-shortcode themedo-blog-shortcode-%s themedo-blog-archive %s themedo-blog-%s', $this->blog_sc_counter, $blog_layout, self::$args['scrolling'] );

		if ( ! self::$args['thumbnail'] ) {
			$attr['class'] .= ' themedo-blog-no-images';
		}
		...
		if ( self::$args['blog_grid_column_spacing'] == '0' || 
			...
			$attr['class'] .= ' themedo-no-col-space';
		}
	...
	function posts_container_attr() {
		...
		if ( self::$args['load_more'] ) {
			$load_more = ' themedo-posts-container-load-more';
		}
		
		$attr['class'] = sprintf( 'themedo-posts-container themedo-posts-container-%s%s', self::$args['scrolling'], $load_more );		
		
		$attr['data-pages'] = $this->query->max_num_pages;
		
		if ( self::$args['layout'] == 'grid' ) {
			 $attr['class'] .= sprintf( ' themedo-blog-layout-grid themedo-blog-layout-grid-%s isotope', self::$args['blog_grid_columns'] );
	...
	function wrap_loop_open() {
		...
		if ( self::$args['layout'] == 'timeline' ) {
			...
			$wrapper = sprintf( '<div %s><i %s></i></div>', themedoCore_Plugin::attributes( 'themedo-timeline-icon' . $class_timeline_icon ), themedoCore_Plugin::attributes( 'themedo-icon-bubbles' ) );
			$wrapper .= sprintf( '<div %s>', themedoCore_Plugin::attributes( 'themedo-blog-layout-timeline themedo-clearfix' ) );
			$wrapper .= '<div class="themedo-timeline-line"></div>';
		}
	...
	function wrap_loop_close() {
		...
		if ( self::$args['layout'] == 'grid' ) {
			$wrapper .= '<div class="themedo-clearfix"></div>';
		}
	...
	function before_loop() {
		...
		echo sprintf( '<div %s>', themedoCore_Plugin::attributes( 'blog-shortcode-loop' ) ) . "\n";
	...
	function loop_attr() {
		...
		// In any other case, add the correct post class
			...
			$extra_classes[] = sprintf( 'themedo-post-%s', self::$args['layout'] );
		}
		...
		if ( self::$args['layout'] == 'timeline' ) {
			...
			if ( ( $this->post_count % 2 ) > 0 ) {
				$timeline_align = ' themedo-left-column';
			} else {
				$timeline_align = ' themedo-right-column';
			}

			$extra_classes[] = 'themedo-clearfix' . $timeline_align;
		}
	...
	function get_slideshow() {
		...
		if ( ! post_password_required( get_the_ID() ) && function_exists( 'avalon_td_plugins_loaded' ) ) {

			$slideshow = array(
				'images' => $this->get_post_thumbnails( get_the_ID(), $avalon_td_option['posts_slideshow_number'] )
			);
	...
	function get_post_thumbnails( $post_id, $count = '' ) {
		...
		$i = 2;
		while ( $i <= $avalon_td_option['posts_slideshow_number'] ) {
	...
	function loop_header() {
		...
		if ( self::$args['layout'] == 'medium-alternate' || 
			...
			$pre_title_content .= sprintf( '<div %s>', themedoCore_Plugin::attributes( 'themedo-date-and-formats' ) );
			ob_start();
			do_action( 'avalon_td_blog_shortcode_date_and_format' );
			...
			if ( self::$args['meta_all'] ) {
				$meta_data .= avalon_td_render_post_metadata( 'alternate', $this->meta_info_settings );
			}
		}
		...
		if ( self::$args['layout'] == 'grid' ||
			...
			if ( self::$args['meta_info_combined'] > 0 &&
				...
				$content_sep = sprintf( '<div %s></div>', themedoCore_Plugin::attributes('themedo-content-sep' ) );
			}
			...
			if ( self::$args['meta_all'] ) {
				$meta_data .= avalon_td_render_post_metadata( 'grid_timeline', $this->meta_info_settings );
			}
			
			$pre_title_content .=  sprintf( '<div %s%s>', themedoCore_Plugin::attributes( 'themedo-post-content-wrapper' ), $content_wrapper_styles );
		}
		
		$pre_title_content .=  sprintf( '<div %s>', themedoCore_Plugin::attributes( 'themedo-post-content post-content' ) );
		
		if ( self::$args['title'] ) {
			if ( self::$args['title_link'] ) {
				$link_target = '';
				if( avalon_td_get_page_option( 'link_icon_target', get_the_ID() ) == 'yes' ||
					avalon_td_get_page_option( 'post_links_target', get_the_ID() ) == 'yes' ) {
		...
		if ( self::$args['layout'] == 'timeline' ) {
			$pre_title_content .= sprintf( '<div %s></div><div %s></div>', themedoCore_Plugin::attributes( 'themedo-timeline-circle' ), themedoCore_Plugin::attributes( 'themedo-timeline-arrow' ) );
		}

		$html = sprintf( '%s<h2 %s>%s</h2>%s%s', $pre_title_content, themedoCore_Plugin::attributes( 'blog-shortcode-post-title' ), $link, $meta_data, $content_sep );
	...
	function post_title_attr() {
		...
		if ( ! $avalon_td_option['disable_date_rich_snippet_pages'] ) {
	...
	function loop_footer() {
		...
		if ( self::$args['layout'] == 'grid' || 
			...
			if ( self::$args['meta_info_combined'] > 0 ) {
				...
				echo sprintf( '<div class="themedo-meta-info">%s</div>', $inner_content );
			}
		}
		
		echo '</div>';
		echo '<div class="themedo-clearfix"></div>';
		
		if ( self::$args['meta_info_combined'] > 0 && self::$args['layout'] == 'large' ||
			 self::$args['meta_info_combined'] > 0 && self::$args['layout'] == 'medium'
		) {
			echo sprintf( '<div class="themedo-meta-info">%s%s</div>', avalon_td_render_post_metadata( 'standard', $this->meta_info_settings ), $this->read_more() );
		}		
	...
	function add_date_box() {
		...
		$inner_content = sprintf( '<div %s>', themedoCore_Plugin::attributes( 'themedo-date-box updated' ) );

		$inner_content .= sprintf( '<span %s>%s</span>', themedoCore_Plugin::attributes( 'themedo-date' ), get_the_time( $avalon_td_option['alternate_date_format_day'] ) );
		$inner_content .= sprintf( '<span %s>%s</span>', themedoCore_Plugin::attributes( 'themedo-month-year' ), get_the_time( $avalon_td_option['alternate_date_format_month_year'] ) );
	...
	function add_format_box() {
		...
		$inner_content = sprintf( '<div %s>
								  	<i %s></i>
								  </div>', themedoCore_Plugin::attributes( 'themedo-format-box' ), themedoCore_Plugin::attributes( 'themedo-icon-' . $format_class ) );
	...
	function timeline_date( $date_params ) {
		...
		if ( $args['prev_post_month'] != $args['post_month'] ||
			...
			$inner_content .= sprintf( '<h3 %s>%s</h3>', themedoCore_Plugin::attributes( 'themedo-timeline-date' ), get_the_date( $avalon_td_option['timeline_date_format'] ) );
			$inner_content .= '<div class="themedo-collapse-month">';
		}
	...
	function grid_timeline_comments() {
		...
		if ( self::$args['meta_comments'] ) {
			...
			$comments_icon = sprintf( '<i %s></i>&nbsp;', themedoCore_Plugin::attributes( 'themedo-icon-bubbles' ) );
			
			if ( ! post_password_required( get_the_ID() ) ) {		
				ob_start();
				comments_popup_link( $comments_icon . __( '0', 'themedo-core' ), $comments_icon . __( '1', 'themedo-core' ), $comments_icon . __( '%', 'themedo-core' ) );
				$comments = ob_get_contents();
				ob_get_clean();
			} else {
				$comments = sprintf( '<i class="themedo-icon-bubbles"></i>&nbsp;%s', __( 'Protected', 'themedo-core' ) );					
			}
			
			$inner_content = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'themedo-alignright' ), $comments );
		...
		if ( self::$args['meta_link'] ) {
			...
			if ( self::$args['meta_read'] ) {
				...
				if ( self::$args['layout'] == 'grid' || 
					...
					$read_more_wrapper_class = 'themedo-alignleft';
				} else {
					$read_more_wrapper_class = 'themedo-alignright';
				}
				...
				$inner_content .= sprintf( '<div %s><a class="themedo-read-more" href="%s"%s>%s</a></div>', themedoCore_Plugin::attributes( $read_more_wrapper_class ),  get_permalink(), $link_target,  apply_filters( 'avalon_td_blog_read_more_link', __( 'Read More', 'themedo-core' ) ) );

				if ( self::$args['layout'] == 'large-alternate' || 
					 self::$args['layout'] == 'medium-alternate'
				) {
					$inner_content = sprintf( '<div class="themedo-meta-info">%s</div>', $inner_content );
				}
	...
	function loop_content() {
		...
		$content = avalon_td_get_post_content( '', self::$args['excerpt'], self::$args['excerpt_words'], self::$args['strip_html'] );
		
		$content = sprintf( '<div class="themedo-post-content-container">%s</div>', $content );
...
add_action('pre_get_posts', 'avalon_td_query_offset', 1 );
function avalon_td_query_offset( &$query ) {
...
add_filter('found_posts', 'avalon_td_adjust_offset_pagination', 1, 2 );
function avalon_td_adjust_offset_pagination( $found_posts, $query ) {
