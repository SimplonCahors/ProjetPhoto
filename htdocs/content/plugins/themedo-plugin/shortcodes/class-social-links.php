...
class themedoSC_SocialLinks {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_social-links-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_social-links-shortcode-social-networks', array( $this, 'social_networks_attr' ) );	
		add_filter( 'avalon_td_attr_social-links-shortcode-icon', array( $this, 'icon_attr' ) );	
	...
	function render( $args, $content = '') {
		...
		$defaults = themedoCore_Plugin::set_shortcode_defaults(
			array(
				'class'					=> '',			
				'id'					=> '',
				'icons_boxed'			=> strtolower( $avalon_td_option['social_links_boxed'] ),
				'icons_boxed_radius' 	=> strtolower( $avalon_td_option['social_links_boxed_radius'] ),
				'icon_colors'			=> strtolower( $avalon_td_option['social_links_icon_color'] ),
				'box_colors'			=> strtolower( $avalon_td_option['social_links_box_color'] ),
				'icon_order'			=> '',
				'show_custom'			=> 'no',
				'alignment'	  		=> '',
				'tooltip_placement'		=> strtolower( $avalon_td_option['social_links_tooltip_placement'] ),
		...
		//$social_networks = themedoCore_Plugin::order_array_like_array( $social_networks, $icon_order );
		...
		foreach( $social_networks as $network => $link ) {
			$custom = '';
			if( $network == 'custom' ) {
				$custom = sprintf( '<img src="%s" alt="%s" />', $avalon_td_option['custom_icon_image'], $avalon_td_option['custom_icon_name'] );
				
				$network = 'custom_' . $avalon_td_option['custom_icon_name'];
			...
			$icons .= sprintf( '<a %s>%s</a>', themedoCore_Plugin::attributes( 'social-links-shortcode-icon', $icon_options ), $custom );
			$i++;
		}
			
		$html = sprintf( '<div %s><div %s>%s</div></div>', themedoCore_Plugin::attributes( 'social-links-shortcode' ), themedoCore_Plugin::attributes( 'social-links-shortcode-social-networks' ), $icons );
	...
	function attr() {
		...
		$attr['class'] = 'themedo-social-links';
	...
	function get_social_links_array() {
		...
		if( self::$args['show_custom'] ) {
			$social_links_array['custom'] = $avalon_td_option['custom_icon_link'];
		}
