...
class themedoSC_Imageframe {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_imageframe-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_imageframe-shortcode-link', array( $this, 'link_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$defaults = themedoCore_Plugin::set_shortcode_defaults(
			array(
				...
				'align' 				=> '',
				'bordercolor' 			=> '',
				'borderradius' 			=> $avalon_td_option['imageframe_border_radius'],
				'bordersize' 			=> $avalon_td_option['imageframe_border_size'],
				'hide_on_mobile'		=> 'no',
				'lightbox' 				=> 'no',
				'lightbox_image'		=> '',
				'link'					=> '',
		...
		if ( ! $bordercolor ) {
			$bordercolor = $avalon_td_option['imgframe_border_color'];
		}

		if ( ! $stylecolor ) {
			$stylecolor = $avalon_td_option['imgframe_style_color'];
		}
		
		$rgb = themedoCore_Plugin::hex2rgb( $stylecolor );
		...
		if ( array_key_exists( '2', $src ) ) {
			...
			$image_id = themedoCore_Plugin::get_attachment_id_from_url( $image_url );
		...
		if( $lightbox == 'yes' ) {
			...
			$output = sprintf( '<a %s>%s</a>', themedoCore_Plugin::attributes( 'imageframe-shortcode-link' ), do_shortcode( $content ) );
		} elseif( $link ) {
			$output = sprintf( '<a %s>%s</a>', themedoCore_Plugin::attributes( 'imageframe-shortcode-link' ), do_shortcode( $content ) );
		}

		$html = sprintf( '<span %s>%s</span>', themedoCore_Plugin::attributes( 'imageframe-shortcode' ), $output );
		...
		if( $hover_type == 'liftup' ) {
			...
			$html = sprintf( '<div %s>%s%s</div>', themedoCore_Plugin::attributes( $liftup_classes ), $liftup_styles, $html );
		}

		if( $align == 'center' ) {
			$html = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'imageframe-align-center' ), $html );
		}
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'themedo-imageframe imageframe-%s imageframe-%s', self::$args['style'], $this->imageframe_counter );
		...
		if( self::$args['hide_on_mobile'] == 'yes' ) {
			$attr['class'] .= ' themedo-hide-on-mobile';
		}
	...
	function link_attr() {
		...
		if( self::$args['lightbox'] == 'yes' ) {
			...
			$attr['class'] = 'themedo-no-lightbox';		
