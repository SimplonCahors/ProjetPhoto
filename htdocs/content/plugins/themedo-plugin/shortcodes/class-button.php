...
class themedoSC_Button {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_button-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_button-shortcode-icon-divder', array( $this, 'icon_divider_attr' ) );
		add_filter( 'avalon_td_attr_button-shortcode-icon', array( $this, 'icon_attr' ) );
		add_filter( 'avalon_td_attr_button-shortcode-button-text', array( $this, 'button_text_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$defaults = themedoCore_Plugin::set_shortcode_defaults( 
			array(
				'class'			   		=> '',		
				'id'				  	=> '',
				'accent_color'			=> strtolower( $avalon_td_option['button_accent_color'] ),
				'accent_hover_color'	=> strtolower( $avalon_td_option['button_accent_hover_color'] ),
				'bevel_color'			=> strtolower( $avalon_td_option['button_bevel_color'] ),		
				'border_width'			=> strtolower( $avalon_td_option['button_border_width'] ),
				'color'			   		=> 'default',
				'gradient_colors'	  	=> strtolower( $avalon_td_option['button_gradient_top_color'] ) . '|' . strtolower( $avalon_td_option['button_gradient_bottom_color'] ),
				'icon'					=> '',
				'icon_divider'			=> 'no',			
				'icon_position'			=> 'left',			
				'link'					=> '',
				'modal'					=> '',
				'shape'					=> strtolower( $avalon_td_option['button_shape'] ),
				'size'					=> strtolower( $avalon_td_option['button_size'] ),
				'target'			  	=> '_self',
				'title'			   		=> '',
				'type'					=> strtolower( $avalon_td_option['button_type'] ),
				'alignment'	  			=> '',
				'animation_type'	  	=> '',
				...
				'gradient_hover_colors'	=> strtolower( $avalon_td_option['button_gradient_top_color_hover'] ) . '|' . strtolower( $avalon_td_option['button_gradient_bottom_color_hover'] ),
			), $args 
		);

		if( $defaults['color'] == 'default' ) {
			$defaults['accent_color'] = strtolower( $avalon_td_option['button_accent_color'] );
			$defaults['accent_hover_color']	= strtolower( $avalon_td_option['button_accent_hover_color'] );
			$defaults['bevel_color'] = strtolower( $avalon_td_option['button_bevel_color'] );
			$defaults['gradient_colors'] = strtolower( $avalon_td_option['button_gradient_top_color'] ) . '|' . strtolower( $avalon_td_option['button_gradient_bottom_color'] );
			$defaults['gradient_hover_colors'] = strtolower( $avalon_td_option['button_gradient_top_color_hover'] ) . '|' . strtolower( $avalon_td_option['button_gradient_bottom_color_hover'] );
		}
		...
		if( ( $color == 'custom' || $color == 'default' ) && 
			...
			if( $general_styles ) {
				$styles .= sprintf( '.themedo-button.button-%s{%s}', $this->button_counter, $general_styles );
			}
			
			if( $button_3d_styles ) {
				$styles .= sprintf( '.themedo-button.button-%s.button-3d{%s}.button-%1$s.button-3d:active{%2$s}', $this->button_counter, $button_3d_styles );
			}
			
			if( $hover_styles ) {
				$styles .= sprintf( '.themedo-button.button-%s:hover,.themedo-button.button-%s:focus,.themedo-button.button-%s:active{%s}', $this->button_counter, $this->button_counter, $this->button_counter, $hover_styles );
			}				
			...
			if( $gradient_colors ) {
				...
				$styles .= sprintf( '.themedo-button.button-%s{%s}', $this->button_counter, $gradient_styles );				
			}
			...
			if( $gradient_hover_colors ) {
				...
				$styles .= sprintf( '.themedo-button.button-%s:hover,.button-%s:focus,.themedo-button.button-%s:active{%s}', $this->button_counter, $this->button_counter, $this->button_counter, $gradient_hover_styles );
			}			
		...
		$icon_html = '';
		if( $icon ) {
			$icon_html = sprintf( '<i %s></i>', themedoCore_Plugin::attributes( 'button-shortcode-icon' ) );
		
			if( $icon_divider == 'yes' ) {
				$icon_html = sprintf( '<span %s>%s</span>', themedoCore_Plugin::attributes( 'button-shortcode-icon-divder' ), $icon_html );
			}		
		}

		$button_text = sprintf( '<span %s>%s</span>', themedoCore_Plugin::attributes( 'button-shortcode-button-text' ), do_shortcode( $content ) );
		...
		$html = sprintf( '%s<a %s>%s</a>', $styles, themedoCore_Plugin::attributes( 'button-shortcode' ), $inner_content );

		// Add wrapper to the button for alignment and scoped styling 
		if ( $alignment ) {
			$alignment = ' themedo-align' . $alignment;
		}
		
		$html = sprintf( '<div class="themedo-button-wrapper%s">%s</div>', $alignment, $html );
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'themedo-button button-%s button-%s button-%s button-%s button-%s', self::$args['type'], self::$args['shape'], self::$args['size'], self::$args['color'], $this->button_counter );
	...
	function icon_attr() {
		...
		$attr['class'] = sprintf( 'fa %s', themedoCore_Plugin::font_awesome_name_handler( self::$args['icon'] ) );
	...
	function button_text_attr() {
		...
		if( self::$args['icon'] &&
			self::$args['icon_divider'] == 'yes' 
 			...
 			$attr['class'] = sprintf( 'themedo-button-text-%s', self::$args['icon_position'] );
 		} else {
 			$attr['class'] = 'themedo-button-text';
 		}
