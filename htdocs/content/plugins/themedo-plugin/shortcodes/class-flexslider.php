...
class themedoSC_Flexslider {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_flexslider-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_flexslider-shortcode-slides-container', array( $this, 'slides_container_attr' ) );
		add_filter( 'avalon_td_attr_flexslider-shortcode-caption', array( $this, 'caption_attr' ) );
		add_filter( 'avalon_td_attr_flexslider-shortcode-title-container', array( $this, 'title_container_attr' ) );
		add_filter( 'avalon_td_attr_flexslider-shortcode-thumbnails', array( $this, 'thumbnails_attr' ) );
	...
	function render( $args, $content = '' ) {
		...
		$slides_html = sprintf( '<ul %s>%s</ul>', themedoCore_Plugin::attributes( 'flexslider-shortcode-slides-container' ), $slider );


		$html = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'flexslider-shortcode' ), $slides_html );

		if( $layout == 'attachments' ) {
			$thumbnails_html = '';
			$html .= sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'flexslider-shortcode-thumbnails' ), $thumbnails_html );
		}
			...
			if( $query->have_posts() ):
				...
				while( $query->have_posts() ): $query->the_post();
					...
					if( isset( $meta['caption'] ) && $meta['caption'] ) {
						$caption = sprintf( '<p %s>%s</p>', themedoCore_Plugin::attributes( 'flexslider-shortcode-caption' ), $meta['caption'] );
					}

					$html .= sprintf( '<li>%s</li>', avalon_td_get_post_content( '', 'yes', self::$args['excerpt'], true ) . $caption );
	...
	function posts() {
		...
		if( $query->have_posts() ):
			...
			while( $query->have_posts() ): $query->the_post();
				...
				$container = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'flexslider-shortcode-title-container' ), $title );
	...
	function posts_excerpt() {
		...
		if( $query->have_posts() ):
			...
			while( $query->have_posts() ): $query->the_post();
				...
				$title = sprintf( '<h2><a href="%s">%s</a></h2>', get_permalink(), get_the_title() );
				$excerpt = sprintf( '%s', avalon_td_get_post_content( '', 'yes', self::$args['excerpt'], true ) );

				$container = sprintf( '<div %s><div %s>%s</div></div>', themedoCore_Plugin::attributes( 'flexslider-shortcode-title-container' ), 
									  themedoCore_Plugin::attributes( 'excerpt-container' ), $title . $excerpt );
	...
	function attr() {
		...
		$attr['class'] = 'themedo-flexslider themedo-flexslider-loading flexslider-' . self::$args['layout'];
