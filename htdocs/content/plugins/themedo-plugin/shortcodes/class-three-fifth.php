...
class themedoSC_ThreeFifth {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_three-fifth-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_three-fifth-shortcode-wrapper', array( $this, 'wrapper_attr' ) );		
	...
	function render( $args, $content = '') {
		...
		$defaults =	shortcode_atts(
			array(
				...
				'hide_on_mobile'		=> 'no',			
				'last'  				=> 'no',
				'margin_top'			=> $avalon_td_option['col_top_margin'],
				'margin_bottom'			=> $avalon_td_option['col_bottom_margin'],
				'padding'				=> '',
				'spacing'				=> 'yes',
				'animation_type' 		=> '',
		...
		if( $defaults['margin_top'] == '' ) {
			$defaults['margin_top'] = $avalon_td_option['col_top_margin'];
		}

		if( $defaults['margin_bottom'] == '' ) {
			$defaults['margin_bottom'] = $avalon_td_option['col_bottom_margin'];
		}
		...
		if ( $center_content == 'yes' ) {
			$inner_content = sprintf( '<div class="themedo-column-table"><div class="themedo-column-tablecell">%s</div></div>', $inner_content );
		}

		// Setup the main markup
		$html = sprintf( '<div %s><div %s>%s</div></div>%s', themedoCore_Plugin::attributes( 'three-fifth-shortcode' ), themedoCore_Plugin::attributes( 'three-fifth-shortcode-wrapper' ), $inner_content, $clearfix );
	...
	function attr() {
		...
		$attr['class'] = 'themedo-three-fifth themedo-layout-column';
		...
		$attr['class'] .= ' themedo-spacing-' . self::$args['spacing'];
	...
	function wrapper_attr() {
		...
		$attr['class'] = 'themedo-column-wrapper';
