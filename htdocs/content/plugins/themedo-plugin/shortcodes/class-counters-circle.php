<?php
class themedoSC_CountersCircle {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_counters-circle-shortcode', array( $this, 'parent_attr' ) );
		add_shortcode( 'counters_circle', array( $this, 'render_parent' ) );

		add_filter( 'avalon_td_attr_counter-circle-shortcode', array( $this, 'child_attr' ) );
		add_filter( 'avalon_td_attr_counter-circle-wrapper-shortcode', array( $this, 'child_wrapper_attr' ) );
	...
	function render_parent( $args, $content = '' ) {
		...
		$html = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'counters-circle-shortcode' ), do_shortcode( $content ) );
	...
	function parent_attr() {
		...
		$attr['class'] = 'themedo-counters-circle counters-circle';
	...
	function render_child( $args, $content = '' ) {
		...
		$defaults =	themedoCore_Plugin::set_shortcode_defaults(
			array(
				...
				'id'			=> '',
				'countdown'		=> 'no',
				'filledcolor' 	=> strtolower( $avalon_td_option['counter_filled_color'] ),
				'unfilledcolor' => strtolower( $avalon_td_option['counter_unfilled_color'] ),
				'scales'		=> 'no',
				'size' 			=> '220',
				'speed' 		=> '1500',
				'value' 		=> '1',				
		...
		$output = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'counter-circle-shortcode' ), do_shortcode( $content ) );

		$html = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'counter-circle-wrapper-shortcode' ), $output );
	...
	function child_attr() {
		...
		$attr['class'] = 'themedo-counter-circle counter-circle counter-circle-content';
