...
class themedoSC_SharingBox {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_sharingbox-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_sharingbox-shortcode-tagline', array( $this, 'tagline_attr' ) );
		add_filter( 'avalon_td_attr_sharingbox-shortcode-social-networks', array( $this, 'social_networks_attr' ) );
		add_filter( 'avalon_td_attr_sharingbox-shortcode-icon', array( $this, 'icon_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$defaults = themedoCore_Plugin::set_shortcode_defaults(
			array(
				'class' 				=> '',
				'id' 					=> '',			
				'backgroundcolor' 		=> strtolower( $avalon_td_option['sharing_box_bg_color'] ),
				'description'			=> '',
				'icon_colors'			=> strtolower( $avalon_td_option['sharing_social_links_icon_color'] ),
				'box_colors'			=> strtolower( $avalon_td_option['sharing_social_links_box_color'] ),	
				'icons_boxed'			=> strtolower( $avalon_td_option['sharing_social_links_boxed'] ),
				'icons_boxed_radius'	=> strtolower( $avalon_td_option['sharing_social_links_boxed_radius'] ),		
				'link' 					=> '',
				'pinterest_image' 		=> '',
				'social_networks'		=> $this->get_theme_options_settings(),
				'tagline' 				=> __( 'Share This Story, Choose Your Platform!', 'themedo-core' ),
				'tagline_color'			=> strtolower( $avalon_td_option['sharing_box_tagline_text_color'] ),
				'title' 				=> '',
				'tooltip_placement'		=> strtolower( $avalon_td_option['sharing_social_links_tooltip_placement'] ),
			), $args
		);	
		...
		if( isset( $avalon_td_option['social_sorter'] ) && $avalon_td_option['social_sorter'] ) {
			$order = $avalon_td_option['social_sorter'];
			...
			if( isset( $ordered_array ) && $ordered_array && is_array( $ordered_array ) ) {
				...
				foreach( $ordered_array as $key => $field_order ) {
					...
					$find_the_field = $avalon_td_option['social_sorter_' . $field_order_number];
					$field_name = str_replace( '_link', '', $avalon_td_option['social_sorter_' . $field_order_number] );
		...
		for( $i = 0; $i < count( $social_networks ); $i++ ) {
			...
			$icons .= sprintf( '<a %s></a>', themedoCore_Plugin::attributes( 'sharingbox-shortcode-icon', $icon_options ) );
		}

		$html = sprintf( '<div %s><h4 %s>%s</h4><div %s>%s</div></div>', themedoCore_Plugin::attributes( 'sharingbox-shortcode' ), themedoCore_Plugin::attributes( 'sharingbox-shortcode-tagline' ), 
						 $tagline, themedoCore_Plugin::attributes( 'sharingbox-shortcode-social-networks' ), $icons );
	...
	function attr() {
		...
		$attr['class'] = 'share-box themedo-sharing-box';
	...
	function social_networks_attr() {
		...
		$attr['class'] = 'themedo-social-networks';
	...
	function icon_attr( $args ) {
		...
		$attr['class'] = sprintf( 'themedo-social-network-icon themedo-tooltip themedo-%s themedo-icon-%s', $args['social_network'], $args['social_network'] );	
		...
		if( $avalon_td_option['social_icons_new'] ) {
			$target = '_blank';
		} else {
			$target = '_self';
		}
		...
		if( $avalon_td_option['nofollow_social_links'] ) {
			$attr['rel'] = 'nofollow';
		}
	...
	function get_theme_options_settings() {
		...
		$social_media = '';
			...
			if( $avalon_td_option['sharing_facebook'] ) {
				$social_media .= 'facebook|';
			}

			if( $avalon_td_option['sharing_twitter'] ) {
				$social_media .= 'twitter|';
			}

			if( $avalon_td_option['sharing_linkedin'] ) {
				$social_media .= 'linkedin|';
			}
			
			if( $avalon_td_option['sharing_reddit'] ) {
				$social_media .= 'reddit|';
			}
			
			if( $avalon_td_option['sharing_tumblr'] ) {
				$social_media .= 'tumblr|';
			}
			
			if( $avalon_td_option['sharing_google'] ) {
				$social_media .= 'googleplus|';
			}
			
			if( $avalon_td_option['sharing_pinterest'] ) {
				$social_media .= 'pinterest|';
			}
			
			if( $avalon_td_option['sharing_vk'] ) {
				$social_media .= 'vk|';
			}			

			if( $avalon_td_option['sharing_email'] ) {
				$social_media .= 'mail|';
			}
