...
class themedoSC_Alert {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_alert-shortcode', array( $this, 'attr' ) );
		add_filter( 'avalon_td_attr_alert-shortcode-icon', array( $this, 'icon_attr' ) );
		add_filter( 'avalon_td_attr_alert-shortcode-button', array( $this, 'button_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$html = sprintf( '<div %s>', themedoCore_Plugin::attributes( 'alert-shortcode' ) ) . "\n";
		$html .= sprintf( '  <button %s>&times;</button>', themedoCore_Plugin::attributes( 'alert-shortcode-button' ) ) . "\n";
		if( $icon && $icon != 'none' ) {
			$html .= sprintf( '<span %s><i %s></i></span>', themedoCore_Plugin::attributes( 'alert-icon' ), themedoCore_Plugin::attributes( 'alert-shortcode-icon' ) );
		}
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'themedo-alert alert %s alert-dismissable alert-%s', self::$args['type'], $this->alert_class );
	...
	function icon_attr() {
		...
		$attr['class'] = sprintf( 'fa fa-lg %s', themedoCore_Plugin::font_awesome_name_handler( self::$args['icon'] ) );
