...
class themedoSC_ContentBoxes {
	...
	public function __construct() {
		...
		add_filter( 'avalon_td_attr_content-box-shortcode', array( $this, 'child_attr' ) );
		add_filter( 'avalon_td_attr_content-box-shortcode-content-wrapper', array( $this, 'content_wrapper_attr' ) );
		add_filter( 'avalon_td_attr_content-box-shortcode-heading-wrapper', array( $this, 'heading_wrapper_attr' ) );
		add_filter( 'avalon_td_attr_content-box-shortcode-content-container', array( $this, 'content_container_attr' ) );
		
		add_filter( 'avalon_td_attr_content-box-shortcode-link', array( $this, 'link_attr' ) );
		add_filter( 'avalon_td_attr_content-box-shortcode-icon-parent', array( $this, 'icon_parent_attr' ) );
		add_filter( 'avalon_td_attr_content-box-shortcode-icon-wrapper', array( $this, 'icon_wrapper_attr' ) );
		add_filter( 'avalon_td_attr_content-box-shortcode-icon', array( $this, 'icon_attr' ) );
		add_filter( 'avalon_td_attr_content-box-shortcode-timeline', array( $this, 'timeline_attr' ) );
		add_filter( 'avalon_td_attr_content-box-heading', array( $this, 'content_box_heading_attr' ) );
		add_shortcode( 'content_box', array( $this, 'render_child' ) );

		add_filter( 'avalon_td_attr_content-boxes-shortcode', array( $this, 'parent_attr' ) );
	...
	function render_parent( $args, $content = '') {
		...
		$defaults =	themedoCore_Plugin::set_shortcode_defaults(
			array(
				'class'					=> '',			
				'id' 					=> '',
				'backgroundcolor' 		=> $avalon_td_option['content_box_bg_color'],
				'columns' 				=> '',
				'circle'				=> '',
				'iconcolor' 			=> $avalon_td_option['content_box_icon_color'],
				'circlecolor' 			=> $avalon_td_option['content_box_icon_bg_color'],
				'circlebordercolor' 	=> $avalon_td_option['content_box_icon_bg_inner_border_color'],
				'circlebordersize'		=> $avalon_td_option['content_box_icon_bg_inner_border_size'],
				'outercirclebordercolor'=> $avalon_td_option['content_box_icon_bg_outer_border_color'],
				'outercirclebordersize' => $avalon_td_option['content_box_icon_bg_outer_border_size'],
				'icon_circle'			=> $avalon_td_option['content_box_icon_circle'],
				'icon_circle_radius'	=> $avalon_td_option['content_box_icon_circle_radius'],
				'icon_size'				=> $avalon_td_option['content_box_icon_size'],
				'icon_align'			=> '',
				'icon_hover_type'		=> $avalon_td_option['content_box_icon_hover_type'],
				'layout' 				=> 'icon-with-title',
				'margin_top' 			=> $avalon_td_option['content_box_margin_top'],
				'margin_bottom' 		=> $avalon_td_option['content_box_margin_bottom'],
				'title_size'			=> $avalon_td_option['content_box_title_size'],
				'title_color'			=> $avalon_td_option['content_box_title_color'],
				'body_color'			=> $avalon_td_option['content_box_body_color'],
				'link_type'				=> $avalon_td_option['content_box_link_type'],
				'link_area'				=> $avalon_td_option['content_box_link_area'],
				'linktarget'			=> $avalon_td_option['content_box_link_target'],
				'animation_type' 		=> '',
				'animation_delay' 		=> '',
		...
		$html = sprintf( '<div %s>%s<div class="themedo-clearfix"></div></div>', themedoCore_Plugin::attributes( 'content-boxes-shortcode' ), do_shortcode( $content ) );
	...
	function parent_attr() {
		...
		$attr['class'] = sprintf( 'themedo-content-boxes content-boxes columns themedo-columns-%s themedo-content-boxes-%s content-boxes-%s row content-%s', $this->num_of_columns, $this->content_box_counter, self::$parent_args['layout'], self::$parent_args['icon_align'] );
		...
		if( self::$parent_args['animation_delay'] ) {
			...
			$attr['class'] .= ' themedo-delayed-animation';
		}
	...
	function render_child( $args, $content = '') {
		...
		if( $image && 
			...
			$image_id = themedoCore_Plugin::get_attachment_id_from_url( $image );

			if( $image_id ) {
				$alt = get_post_meta( $image_id, '_wp_attachment_image_alt', true );
			}
		
			$icon_output = sprintf( '<div %s><img src="%s" width="%s" height="%s" alt="%s" /></div>', themedoCore_Plugin::attributes( 'content-box-shortcode-icon' ), $image, $image_width, $image_height, $alt );
		} elseif( $icon ) {
			if( $outercirclebordercolor && $outercirclebordersize ) {
				$icon_output = sprintf( '<div %s><span %s><i %s></i></span></div>', themedoCore_Plugin::attributes( 'content-box-shortcode-icon-parent' ), themedoCore_Plugin::attributes( 'content-box-shortcode-icon-wrapper' ), themedoCore_Plugin::attributes( 'content-box-shortcode-icon' ) );
			} else {
				$icon_output = sprintf( '<div %s><i %s></i></div>', themedoCore_Plugin::attributes( 'content-box-shortcode-icon-parent' ), themedoCore_Plugin::attributes( 'content-box-shortcode-icon' ) );
			}
		}

		if( $title ) {
			$title_output = sprintf( '<h2 %s>%s</h2>', themedoCore_Plugin::attributes( 'content-box-heading' ), $title );
		}
		...
		if( $link ) {
			$heading_content = sprintf( '<a %s %s>%s</a>', themedoCore_Plugin::attributes( 'heading-link' ), 
										themedoCore_Plugin::attributes( 'content-box-shortcode-link' ), $heading_content );
		}
		
		if ( $heading_content ) {
			$heading = sprintf( '<div %s>%s</div>', themedoCore_Plugin::attributes( 'content-box-shortcode-heading-wrapper' ), $heading_content );
		}
		...
		if( $link &&  
			...
			if( self::$parent_args['link_type'] == 'text' || self::$parent_args['link_type'] == 'button-bar' ) {
				$link_output = sprintf( '<div class="themedo-clearfix"></div><a %s %s>%s</a><div class="themedo-clearfix"></div>', themedoCore_Plugin::attributes( 'themedo-read-more' ), themedoCore_Plugin::attributes( 'content-box-shortcode-link' ), $linktext );
			} else if( self::$parent_args['link_type'] == 'button' ) {
				$link_output = sprintf( '<div class="themedo-clearfix"></div><a %s>%s</a><div class="themedo-clearfix"></div>', themedoCore_Plugin::attributes( 'content-box-shortcode-link' ), $linktext );
			}
		}

		$content_output = sprintf( '<div class="themedo-clearfix"></div><div %s>%s</div>', themedoCore_Plugin::attributes( 'content-box-shortcode-content-container'), do_shortcode( $content ) . $link_output );
		...
		if( $icon && self::$parent_args['icon_circle'] == 'yes' && self::$parent_args['layout'] == 'timeline-horizontal' && self::$parent_args['columns'] != '1' ) {
			$timeline = sprintf( '<div %s></div>', themedoCore_Plugin::attributes( 'content-box-shortcode-timeline' ) );
		}
		...
		$html = sprintf( '<div %s><div %s>%s%s</div></div>', themedoCore_Plugin::attributes( 'content-box-shortcode' ), 
						 themedoCore_Plugin::attributes( 'content-box-shortcode-content-wrapper' ), $output, $timeline );

		$clearfix_test = $this->column_counter / $this->num_of_columns;

		if ( is_int( $clearfix_test ) ) {
			$html .= '<div class="themedo-clearfix"></div>';
		}
	...
	function child_attr() {
		...
		$attr['class'] = sprintf( 'themedo-column content-box-column content-box-column-%s col-lg-%s col-md-%s col-sm-%s', $this->column_counter, $columns, $columns, $columns );
		$attr['style'] = '';
		...
		if( $this->num_of_columns == '5'  ) {
			$attr['class'] = 'themedo-column content-box-column col-lg-2 col-md-2 col-sm-2';
		}
	...
	function link_attr() {
		...
		if( self::$parent_args['link_type'] == 'button' ) {
			$attr['class'] .= sprintf( 'themedo-read-more-button themedo-button themedo-button-default themedo-button-%s themedo-button-%s themedo-button-%s', strtolower( $avalon_td_option['button_size'] ), strtolower( $avalon_td_option['button_shape'] ), strtolower( $avalon_td_option['button_type'] ) );
		}
	...
	function icon_attr() {
		...
		if( self::$child_args['image'] ) {
			...
			$attr['class'] = sprintf( 'fa fontawesome-icon %s', themedoCore_Plugin::font_awesome_name_handler( self::$child_args['icon'] ) );
			...
			$attr['style'] .= sprintf( 'font-size:%spx;', themedoCore_Plugin::strip_unit( self::$parent_args['icon_size'] ) );
		}
	...
	function content_box_heading_attr() {
		...
		if( self::$parent_args['title_size'] ) {
			$font_size = themedoCore_Plugin::strip_unit( self::$parent_args['title_size'] );
