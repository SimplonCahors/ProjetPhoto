...
<div id="pixproof_gallery" class="pixproof_gallery cf  js-pixproof-gallery">
	<ul class="masonry_items">
	<?php
	$idx = 1;
	foreach ( $attachments as $attachment ) {
		if ( 'selected' == self::get_attachment_class( $attachment ) ) {
			$select_label = esc_html__( 'Deselect', 'themedo-core' );
		} else {
			$select_label = esc_html__( 'Select', 'themedo-core' );
		}
		
		//$thumb_img  = wp_get_attachment_image_src( $attachment->ID, $thumbnails_size );
		$thumb_img  = wp_get_attachment_image_src( $attachment->ID, 'avalon_td_thumb-500-9999' );
		...
		<li class="proof-photo  js-proof-photo  gallery-item <?php self::attachment_class( $attachment ); ?>" <?php self::attachment_data( $attachment ); ?>  id="<?php echo $image_id_tag; ?>">
			<div class="proof-photo__container">
				<div class="avalon_td_extra">
