...
class pixproof_Meta_Box {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear' => esc_html__( 'Clear', 'themedo-core' ),
					'defaultString' => esc_html__( 'Default', 'themedo-core' ),
					'pick' => esc_html__( 'Select Color', 'themedo-core' ),
					'current' => esc_html__( 'Current Color', 'themedo-core' ),
				) );
			}
		} else {
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			if ( get_post_type() === 'avalon_td_proofing' ) {
...
function pixproof_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'themedo-core' ) );
