...
class themepacific_Jetpack_Carousel {
	...
	function enqueue_assets( $output ) {
		...
		if ( $this->first_run ) {
			...
			wp_register_script('spin', get_template_directory_uri() . '/inc/gallerytiled/spin.js', 'jquery', '1.0', TRUE);
			wp_register_script('jquery.spin', get_template_directory_uri() . '/inc/gallerytiled/jquery.spin.js', 'jquery', '1.0', TRUE);
			...
			//wp_register_script('jetpack-carousel', get_template_directory_uri() . '/inc/gallerytiled/jetpack-carousel.js', 'jquery', '1.0', TRUE);
			...
			$localize_strings = array(
				...
				'comment'              => __( 'Comment', 'themedo-core' ),
				'post_comment'         => __( 'Post Comment', 'themedo-core' ),
				'loading_comments'     => __( 'Loading Comments...', 'themedo-core' ),
				'download_original'    => sprintf( __( 'View full size <span class="photo-size">%1$s<span class="photo-size-times">&times;</span>%2$s</span>', 'themedo-core' ), '{0}', '{1}' ),
				'no_comment_text'      => __( 'Please be sure to submit some text with your comment.', 'themedo-core' ),
				'no_comment_email'     => __( 'Please provide an email address to comment.', 'themedo-core' ),
				'no_comment_author'    => __( 'Please provide your name to comment.', 'themedo-core' ),
				'comment_post_error'   => __( 'Sorry, but there was an error posting your comment. Please try again later.', 'themedo-core' ),
				'comment_approved'     => __( 'Your comment was approved.', 'themedo-core' ),
				'comment_unapproved'   => __( 'Your comment is in moderation.', 'themedo-core' ),
				'camera'               => __( 'Camera', 'themedo-core' ),
				'aperture'             => __( 'Aperture', 'themedo-core' ),
				'shutter_speed'        => __( 'Shutter Speed', 'themedo-core' ),
				'focal_length'         => __( 'Focal Length', 'themedo-core' ),
			...
			if ( ! isset( $localize_strings['jetpack_comments_iframe_src'] ) || empty( $localize_strings['jetpack_comments_iframe_src'] ) ) {
				...
				if ( $is_logged_in ) {
					$localize_strings['local_comments_commenting_as'] = '<p id="jp-carousel-commenting-as">' . sprintf( __( 'Commenting as %s', 'themedo-core' ), $current_user->data->display_name ) . '</p>';
				} else {
					if ( $comment_registration ) {
						$localize_strings['local_comments_commenting_as'] = '<p id="jp-carousel-commenting-as">' . __( 'You must be <a href="#" class="jp-carousel-comment-login">logged in</a> to post a comment.', 'themedo-core' ) . '</p>';
					} else {
						$required = ( $require_name_email ) ? __( '%s (Required)', 'themedo-core' ) : '%s';
						$localize_strings['local_comments_commenting_as'] = ''
							. '<fieldset><label for="email">' . sprintf( $required, __( 'Email', 'themedo-core' ) ) . '</label> '
							. '<input type="text" name="email" class="jp-carousel-comment-form-field jp-carousel-comment-form-text-field" id="jp-carousel-comment-form-email-field" /></fieldset>'
							. '<fieldset><label for="author">' . sprintf( $required, __( 'Name', 'themedo-core' ) ) . '</label> '
							. '<input type="text" name="author" class="jp-carousel-comment-form-field jp-carousel-comment-form-text-field" id="jp-carousel-comment-form-author-field" /></fieldset>'
							. '<fieldset><label for="url">' . __( 'Website', 'themedo-core' ) . '</label> '
			...
			wp_register_style('jetpack-carousel', get_template_directory_uri().'/inc/gallerytiled/jetpack-carousel.css', array(), '1.0', 'all');
			wp_enqueue_style('jetpack-carousel');
			...
			if( $is_IE )
			{
				...
				if( $version < 9 )
					...
					wp_register_style('jetpack-carousel-ie8fix', get_template_directory_uri().'/inc/gallerytiled/jetpack-carousel-ie8fix.css', array(), '1.0', 'all');
	...
	function get_attachment_comments() {
		...
		if ( ! $attachment_id ) {
			echo json_encode( __( 'Missing attachment ID.', 'themedo-core' ) );
			die();
		}
	...
	function post_attachment_comment() {
		...
		if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce($_POST['nonce'], 'carousel_nonce') )
			die( json_encode( array( 'error' => __( 'Nonce verification failed.', 'themedo-core' ) ) ) );
		...
		if ( empty( $_blog_id ) )
			die( json_encode( array( 'error' => __( 'Missing target blog ID.', 'themedo-core' ) ) ) );

		if ( empty( $_post_id ) )
			die( json_encode( array( 'error' => __( 'Missing target post ID.', 'themedo-core' ) ) ) );

		if ( empty( $comment ) )
			die( json_encode( array( 'error' => __( 'No comment text was submitted.', 'themedo-core' ) ) ) );
		...
		if ( ! comments_open( $_post_id ) )
			die( json_encode( array( 'error' => __( 'Comments on this post are closed.', 'themedo-core' ) ) ) );
		...
		if ( is_user_logged_in() ) {
			...
			if ( empty( $user_id ) )
				die( json_encode( array( 'error' => __( 'Sorry, but we could not authenticate your request.', 'themedo-core' ) ) ) );
		} else {
			$user_id      = 0;
			...
			if ( get_option( 'require_name_email' ) ) {
				if ( empty( $display_name ) )
					die( json_encode( array( 'error' => __( 'Please provide your name.', 'themedo-core' ) ) ) );

				if ( empty( $email ) )
					die( json_encode( array( 'error' => __( 'Please provide an email address.', 'themedo-core' ) ) ) );

				if ( ! is_email( $email ) )
					die( json_encode( array( 'error' => __( 'Please provide a valid email address.', 'themedo-core' ) ) ) );
			}
		}
	...
	function register_settings() {
		add_settings_section('carousel_section', __( 'Image Gallery Carousel', 'themedo-core' ), array( $this, 'carousel_section_callback' ), 'media');

		if ( ! $this->in_jetpack ) {
			add_settings_field('carousel_enable_it', __( 'Enable carousel', 'themedo-core' ), array( $this, 'carousel_enable_it_callback' ), 'media', 'carousel_section' );
			register_setting( 'media', 'carousel_enable_it', array( $this, 'carousel_enable_it_sanitize' ) );
		}

		add_settings_field('carousel_background_color', __( 'Background color', 'themedo-core' ), array( $this, 'carousel_background_color_callback' ), 'media', 'carousel_section' );
		register_setting( 'media', 'carousel_background_color', array( $this, 'carousel_background_color_sanitize' ) );

		add_settings_field('carousel_display_exif', __( 'Metadata', 'themedo-core'), array( $this, 'carousel_display_exif_callback' ), 'media', 'carousel_section' );
		...
		// add_settings_field('carousel_display_geo', __( 'Geolocation', 'themedo-core' ), array( $this, 'carousel_display_geo_callback' ), 'media', 'carousel_section' );
	...
	function carousel_display_exif_callback() {
		$this->settings_checkbox( 'carousel_display_exif', __( 'Show photo metadata (<a href="http://en.wikipedia.org/wiki/Exchangeable_image_file_format" target="_blank">Exif</a>) in carousel, when available.', 'themedo-core' ) );
	}
	...
	function carousel_display_geo_callback() {
		$this->settings_checkbox( 'carousel_display_geo', __( 'Show map of photo location in carousel, when available.', 'themedo-core' ) );
	}
	...
	function carousel_background_color_callback() {
		$this->settings_select( 'carousel_background_color', array( 'black' => __( 'Black', 'themedo-core' ), 'white' => __( 'White', 'themedo-core', 'themedo-core' ) ) );
	}
	...
	function carousel_enable_it_callback() {
		$this->settings_checkbox( 'carousel_enable_it', __( 'Display images in full-size carousel slideshow.', 'themedo-core' ) );
	}
