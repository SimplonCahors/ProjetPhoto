...
class themePacific_Jetpack_Tiled_Gallery {
	...
	public function default_scripts_and_styles() {
		...
		wp_register_script('tiled-gallery', get_template_directory_uri() . '/inc/gallerytiled/tiled-gallery/tiled-gallery.js', 'jquery', '1.0', TRUE);
		wp_enqueue_script( 'tiled-gallery' );
		
		wp_register_style('tiled-gallery', get_template_directory_uri().'/inc/gallerytiled/tiled-gallery/tiled-gallery.css', array(), '1.0', 'all');
	...
	if ( $image_src[1] > $width || $image_src[2] > $height ) {
		...
		$new_img_path = '';
		$imageme = wp_get_image_editor($file_path);
		if (!is_wp_error($imageme)) {
			$imageme->resize($width, $height, $crop);
			$resized_file = $imageme->save($new_img_path);
		}
		
		$new_img_size = getimagesize( $resized_file['path'] );
		$new_img = str_replace( basename( $image_src[0] ), basename( $resized_file['path'] ), $image_src[0] );
		...
		foreach ( $grouper->grouped_images as $row ) {
			...
			foreach( $row->groups as $group ) {
				...
				foreach ( $group->images as $image ) {
					...
					$attachment_url = wp_get_attachment_image_src( $image->ID, 'full' );


 					$img_src = $this->vt_resize(  $image->ID,'' , $image->width, $image->height, true ); 
					$output .= '<div class="tiled-gallery-item tiled-gallery-item-' . esc_attr( $size ) . '"><a class="zoomer" title="'. apply_filters('the_title', $image->post_excerpt) .'" data-source="'. $attachment_url[0] .'" href="'. $attachment_url[0] .'"><img src="' . esc_url( $img_src['url'] ) . '" width="' . esc_attr( $image->width ) . '" height="' . esc_attr( $image->height ) . '" align="left" title="' . esc_attr( $image_title ) . '" /></a>';
	...
	public function square_talavera( $attachments ) {
		...
		foreach( $attachments as $image ) {
			...
			$output .= '<a class="zoomer" title="'. apply_filters('the_title', $image->post_excerpt) .'" data-source="'. $attachment_url[0] .'" href="'. $attachment_url[0] .'"><img style="' . esc_attr( 'margin: ' . $margin . 'px' ) . '" src="' .  $img_src['url']  . '" width=' . esc_attr( $img_size ) . ' height=' . esc_attr( $img_size ) . ' title="' . esc_attr( $image_title ) . '" /></a>';
	...
	function jetpack_gallery_types( $types ) {
		...
		$types['rectangular'] = __( 'Tiled Mosaic', 'themedo-core' );
		$types['square'] = __( 'Square Tiles', 'themedo-core' );
		$types['circle'] = __( 'Circles', 'themedo-core' );
	...
	function settings_api_init() {
		...
		add_settings_field( 'tiled_galleries', __( 'Tiled Galleries', 'themedo-core' ), array( $this, 'setting_html' ), 'media', $section );
	...
	function setting_html() {
		echo '<label><input name="tiled_galleries" type="checkbox" value="1" ' .
			...
			__( 'Display all your gallery pictures in a cool mosaic.', 'themedo-core' ) . '</br></label>';
	}
}
