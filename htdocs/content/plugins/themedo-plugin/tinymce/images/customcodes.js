...
(function() {
	tinymce.create('tinymce.plugins.tfbutton', {
		init : function(ed, url) {
			avalon_td_shortcode_url = url;
		},
		createControl : function(n, cm) {
			switch(n) {
				case 'tfbutton':
					var c = cm.createSplitButton('tfbutton', {
						title : 'Themedo Shortcodes',
						image : avalon_td_shortcode_url+'/tfbutton.png'
					});
					...
					c.onRenderMenu.add(function(c, m) {
						...
						m.add({
							title : 'Content Boxes',
							icon: 'avalon_td_content_boxes',
							onclick : function() {
								tinyMCE.activeEditor.selection.setContent('[content_boxes layout="icon-with-title, icon-on-top, icon-on-side, icon-boxed" iconcolor="" circlecolor="" circlebordercolor="" backgroundcolor=""]<br />[content_box title="Responsive Design" icon="tablet" image="" link="http://themeforest.net/user/themedo" linktarget="" linktext="Learn More"]themedo is fully responsive and can adapt to any screen size. Try resizing your browser window to see the adaptation.[/content_box]<br />[content_box title="Awesome Sliders" icon="thumbs-up" image="" link="http://themeforest.net/user/themedo" linktarget="" linktext="Learn More"]themedo includes the awesome Layer Parallax Slider as well as the popular FlexSlider2. Both are super easy to use![/content_box]<br />[content_box title="Unlimited Colors"  icon="magic" image="" link="http://themeforest.net/user/themedo" linktarget="" linktext="Learn More"]We included a backend color picker for unlimited color options. Anything can be changed, including the gradients![/content_box]<br />[content_box last="yes" title="500+ Google Fonts" icon="beaker" image="" link="http://themeforest.net/user/themedo" linktarget="" linktext="Learn More"]themedo loves fonts, choose from over 500+ Google Fonts. You can change all headings and body copy with ease![/content_box]<br />[/content_boxes]');
							}
						});

						m.add({
							title : 'Counters Circle',
							icon: 'avalon_td_counters_circle',
							onclick : function() {
						...
						m.add({
							title : 'Gallery Block',
							icon: 'avalon_td_gallery_block',
							onclick : function() {
								tinyMCE.activeEditor.selection.setContent('[gallery_block layout="slider" cat_slug="" exclude_cats="" post_count="" order="" offset="" class="" id=""][/gallery_block]');
							}
						});

						m.add({
							title : 'Recent Posts',
							icon: 'avalon_td_recent_posts',
							onclick : function() {
						...
						m.add({
							title : 'Toggles',
							icon: 'avalon_td_toggle',
							onclick : function() {
								tinyMCE.activeEditor.selection.setContent('[toggles][toggle title="Title" open="yes"]...[/toggle][toggle title="Title" open="no"]...[/toggle][toggle title="Title" open="no"]...[/toggle][toggle title="Title" open="no"]...[/toggle][toggle title="Title" open="no"]...[/toggle][/toggles]');
							}
						});

						m.add({
							title : 'Testimonial',
							icon: 'avalon_td_testimonial',
							onclick : function() {
						...
						m.add({
							title : 'Tagline Box',
							icon: 'avalon_td_tagline',
							onclick : function() {
								tinyMCE.activeEditor.selection.setContent('[tagline_box backgroundcolor="" shadow="no" shadowopacity="0.1-1" border="1px" bordercolor="" highlightposition="right, left, top or bottom" link="http://themeforest.net/user/themedo" linktarget="" buttoncolor="e.g. green, darkgreen, orange, blue, red, pink, darkgray, lightgray or leave blank" button="Purchase Now" title="themedo is incredibly responsive, with a refreshingly clean design" description="And it has some awesome features, premium sliders, unlimited colors, advanced theme options and so much more!"][/tagline_box]');
							}
						});

						m.add({
							title : 'Tooltip',
							icon: 'avalon_td_tooltip',
							onclick : function() {
