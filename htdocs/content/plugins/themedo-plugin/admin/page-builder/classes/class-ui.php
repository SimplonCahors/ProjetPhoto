...
if( ! class_exists( 'avalon_td_Core_PageBuilder_UI' ) ) {

	class avalon_td_Core_PageBuilder_UI {
		...
		public function wrap_default_editor() {
			...
			if ( in_array( $screen->id, $allowed_screens) ) {
				...
				$pagebuilder_label 	= __( 'Themedo Page Builder', 'themedo-core' );
				$default_label  	= __( 'Default Editor', 'themedo-core' );
				$status		 	= get_post_meta($post_ID, '_themedoPageBuilderStatus', true);
				echo '<a id="themedo-pb-switch-button" href="javascript:void(0);" class="button-primary" data-active-button="' . $default_label . '" data-inactive-button="' . $pagebuilder_label . '">' . $pagebuilder_label . '</a>';
		...
		public function close_default_editor_wrap() {
			...
			if ( in_array( $screen->id, $allowed_screens) ) {
				...
				echo '<textarea class="themedo-builder-pagecontent">' . $content . '</textarea>';
