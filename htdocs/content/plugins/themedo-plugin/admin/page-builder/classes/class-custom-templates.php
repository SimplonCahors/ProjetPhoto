...
if( ! class_exists( 'avalon_td_Core_Custom_Templates' ) ) {

	class avalon_td_Core_Custom_Templates {
		...
		public function get_custom_templates () {
			...
			$content 		= '<div id="custom_templates_wrapper">
								...
								<h1 class="templates_heading">'.__('Save Template', 'themedo-core').'</h1>
								<div class="save_templates_here">
								<a class="templates_selection" id="avalon_td_save_custom_tpl" 
								href="JavaScript:void(0)">'.__('Save Layout As Template', 'themedo-core').'</a>
								</div>
								</div>
								<div id="custom_templates_right">
								<h1 class="templates_heading">'.__('Load Template', 'themedo-core').'</h1>';
			//get templates data
			$templates 		= get_option( 'avalon_td_custom_templates' );
			...
			if ( $templates != false && count( $templates ) > 0 ) {
				...
				$combinations 	= themedoHelper::generate_column_combinations( count( $templates ), $columns );
				...
				for( $i = 0; $i < $columns; $i++) {
					...
					foreach( $templates as $key => $value ) {
						...
						$content.= ' <div class="hidden_overlay">
										<a href="JavaScript:void(0)" data-id="'.$key.'"  class="avalon_td_load_template">'.__('LOAD', 'themedo-core').'</a>
										<a href="JavaScript:void(0)" data-id="'.$key.'"  class="avalon_td_delete_template">'.__('DELETE', 'themedo-core').'</a>
									 </div>';
						$content.= ' <a class="avalon_td_custom_template templates_selection" href="JavaScript:void(0)">'.$key.'</a>';
						$content.= ' </div>';
		...
		public function get_single_template () {
			...
			$custom_templates = get_option( 'avalon_td_custom_templates' );
		...
		public function save_single_template () {
			...
			if ( $custom_templates === false ) {
				...
				$response 							= add_option( 'avalon_td_custom_templates', $custom_templates, '', 'no' );
			} else {
				
				$custom_templates[ stripslashes($_POST['name']) ] = $model;
				
				$response 							= update_option( 'avalon_td_custom_templates',  $custom_templates );
