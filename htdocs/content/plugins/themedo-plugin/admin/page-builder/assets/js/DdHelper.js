...
(function( $ ) {
        ...
        element.draggable(
            {
                ...
                start: function( event, ui ) {
                    ...
                    //remove all previous themedo-hover-active classes
                    $( '.themedo-hover-active' ).removeClass( 'themedo-hover-active' );
                    ...
                    $( ui.helper ).addClass( 'themedo-dragging-element' );
                ...
                stop: function( event, ui ) {
                    ...
                    //remove themedo-hover-active class from all elements
                    $( '.themedo-hover-active' ).removeClass( 'themedo-hover-active' );
                    ...
                    if ( window.oldParent.length > 9 ) {
                        ...
                        if ( phpClass == 'avalon_td_FullWidthContainer' && cElement.get( 'childrenId' ).length > 0 ) {
        ...
        element.droppable(
            {
                ...
                over: function( event, ui ) {
                    ...
                    if ( DdHelper.isDropingAllowed( ui.helper, dropable ) ) {
                        ...
                        dropable.parent().parent().removeClass( 'themedo-hover-active' );
                        dropable.addClass( 'themedo-hover-active' );
                    }
                },
                ...
                out: function( event, ui ) {
                    ...
                    $( this ).removeClass( 'themedo-hover-active' );
                },
                ...
                drop: function( event, ui ) {
                    ...
                    if ( !dropable.is( '.themedo-hover-active' ) ) return false;
                    ...
                    if ( elementId != 'editor' ) {
                        ...
                        if ( parentId ) {
                            ...
                            if ( parent.get( 'php_class' ) == 'avalon_td_FullWidthContainer' && method == 'prepend' ) {
                    ...
                    if ( phpClass == 'avalon_td_FullWidthContainer' && cElement.get( 'childrenId' ).length > 0 ) {
                        /*//turn off editor tracking first
                         ...
                         var elements = themedoHistoryManager.getAllElementsData();
        ...
        var $dialog = $( '#dialog_form' ).dialog(
            {
                ...
                draggable: false,
                modal: true,
                dialogClass: 'themedob-dialog-form',
                buttons: {
                    "SAVE": function() {
                        tinyMCE.triggerSave();
                ...
                open: function( event, ui ) {
                    $( this ).parent().find( '.ui-dialog-titlebar' ).find( '.themedo-tabs-menu' ).remove();
    ...
    DdHelper.handleElementWidthAndOrder = function( PublishRequest ) {
        ...
        for ( var z = 0; z < editorElements.length; z++ ) {
            ...
            if ( elementId ) {
                ...
                if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 ) {
        ...
        for ( var i = 0; i < tl_editorElements.length; i++ ) {
            ...
            if ( elementId ) {
                ...
                if ( floated_width > 1 ) {
                    if ( typeof editorElement_2 == 'object' && editorElement_2.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement_2.className.indexOf( 'avalon_td_full_width' ) == -1 ) { // if an element behind exists, is a layout column but not a full width container
                        element_2.get( 'subElements' )[0].value = 'yes';
                        floated_width = parseFloat( editorElement.getAttribute( 'data-floated_width' ) );
                    }
                    if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && typeof editorElement_3 != 'object' && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 ) { // if current element is a layout column but not a full width container, forward element doesn't exist.
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                    if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 && typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'avalon_td_full_width' ) > -1 ) { // if current element is a layout colum but not full width and not element is full width column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                    if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'grid_one' ) > -1 ) { // if current element is a one column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                } else if ( floated_width == 1 ) {
                    if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 ) { // if current element is layout column and not a full width container
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    }
                    if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 && typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'avalon_td_full_width' ) > -1 ) { // if current element is a layout colum but not full width and not element is full width column
                    ...
                    if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 && (typeof editorElement_3 != 'object' && (typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'avalon_td_layout_column' ) == -1)) ) { // if current element is a layout column and not full width and forward element element doesn't exist, but if forward element is an object, it is not a layout column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && typeof editorElement_3 != 'object' && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 ) { // if current element is a layout column but not full width and forward element is not an object
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && typeof editorElement_3 == 'object' && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 && editorElement_3.className.indexOf( 'avalon_td_layout_column' ) == -1 ) { // if current element is a layout column but not full width and forward element exists but is not a layout column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 && typeof editorElement_3 == 'object' && editorElement_3.className.indexOf( 'avalon_td_full_width' ) > -1 ) { // if current element is a layout colum but not full width and not element is full width column
                        element.get( 'subElements' )[0].value = 'yes';
                        floated_width = 0;
                    } else {
                        if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 ) { // if current element is a layout column and not full width
                            element.get( 'subElements' )[0].value = 'no';
                        } else {
                            floated_width = 0;
                        }
            ...
            if ( ( i + 1 ) != tl_editorElements.length ) { //so this is not last
                ...
                if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && editorElement.className.indexOf( 'avalon_td_full_width' ) == -1 && tl_editorElements[i + 1].className.indexOf( 'avalon_td_element_box' ) > -1 ) {
                    element.get( 'subElements' )[0].value = 'yes';
                }
            }
        }
        ...
        var full_width_elements = document.querySelectorAll( '#editor > .avalon_td_full_width' );
            ...
            for ( var j = 0; j < tl_editorElements.length; j++ ) {
                ...
                if ( elementId ) {
                    ...
                    if ( ( j + 1 ) != tl_editorElements.length ) { //so this is not last
                        //layout column and next is builder element
                        if ( editorElement.className.indexOf( 'avalon_td_layout_column' ) > -1 && tl_editorElements[j + 1].className.indexOf( 'avalon_td_element_box' ) > -1 ) {
                            element.get( 'subElements' )[0].value = 'yes';
                        }
                    }
                }
    ...
    DdHelper.shortCodestoBuilderElements = function() {
        ...
        if ( window.avalon_td_builder_initial_load ) {
            content = $( '.themedo-builder-pagecontent' ).val();
            window.avalon_td_builder_initial_load = false;
        } else {
            content = $( '#content' ).val();
        }
        ...
        if ( content.length > 0 ) {
            ...
            $.post(
                ajaxurl, data, function( response ) {
                    ...
                    window.avalon_td_builder_fully_loaded = true;
                }
            );
        } else {
            window.avalon_td_builder_fully_loaded = true;
    ...
    DdHelper.loadCustomTabs = function() {
        instance = jQuery( '#themedo-page-builder' ).attr( 'instance' ); //post/page ID
        var data = {
    ...
    DdHelper.showHideLoader = function( state, message ) {
        ...
        if ( state == "show" ) {
            ...
            jQuery( '#avalon_td_loading_section' ).show();
        } else if ( state == "hide" ) {
            jQuery( '#avalon_td_loading_section' ).hide();
        }
    }
    /**
    ...
    DdHelper.RemoveDuplicates = function( refView ) {
        ...
        if ( $( "#" + prentId ).hasClass( "avalon_td_full_width" ) ) {
            app.editor.reRender();
        }
    }
