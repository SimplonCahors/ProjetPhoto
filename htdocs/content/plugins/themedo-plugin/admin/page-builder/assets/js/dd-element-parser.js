...
( function($) { 
	...
	DdElementParser.generateHtml = function (model, settings_lvl) {
        ...
        if( model.get('subElements')[0].id == 'avalon_td_settings_lvl' ) {
        	lvl_exists = true;
		...
		//alert(model.get('subElements').length);
		...
		jQuery.each( model.get('subElements') ,function( key, element ){
			...
			if( element['type'] == ADDMORE ) {
				...
				for (var i = 0; i < elementsCount; i++) {
					...
					output 		+= "<tr class='child-clone-row'><td><a href='#' class='themedo-expand-child'>" + model.get('name') + " Item " + (i + 1) + "<i class='themedoa-plus2'></i></a><div class='child-options' style='display: none;'>";
					...
					jQuery.each (element['elements'][i],function( innerKey, dynamic_element ) {
						...
						if( ChildElement['settings_lvl'] == 'child' && lvl_value == 'parent' ) {
							output 	+= "<div class='clearfix form-element-container themedo-element-child form-element-container-" + ChildElement['type'] + "' style='display: none;'><div class='name-description'>";
						} else {
							output 	+= "<div class='clearfix form-element-container themedo-element-child form-element-container-" + ChildElement['type'] + "'><div class='name-description'>";
						}
					...
					output 		+="<a class='child-clone-row-remove themedo-shortcodes-button' href='JavaScript:void(0)'>Remove</a>";
					output 		+= "</div></td></tr>";
				}

				output 			+= "<tr><td><a id='themedo-child-add' href='JavaScript:void(0)'>" + element['buttonText'] + "</a></td></tr>";
				output 			+= "</table>";
				
			} else {
				output 			+= "<div class='clearfix form-element-container form-element-container-" + element['type'] + "'>";
				output 			+= "<div class='name-description'>";
				if( element['name'] != "undefined" ) { output += "<strong>" + element['name'] + "</strong>"; }
        ...
        if ($hasGroups) {
            output = $orig_output;
            var $tabID = 1;
            $menu = '<ul class="themedo-tabs-menu">';
            var $tabs = '<div class="themedo-tabs">';

            if ($group_html['Default'] != "") {
                $menu += '<li class="current"><a href="#tab-'+$tabID+'">Default</a></li>';
                $tabs += '<div id="tab-'+$tabID+'" class="themedo-tab-content" style="display:block;">'+$group_html['Default']+'</div>';
                $tabID++;
            }
            ...
            for ( var key in $group_html ) {
                if (key != "Default") {
                    ...
                    $tabs += '<div id="tab-'+$tabID+'" class="themedo-tab-content"'+$class+'>'+$group_html[key]+'</div>';
                    $tabID++;
                }
            }
            $menu += "</ul>";
	...
	DdElementParser.parseElementType = function (element) {
		...
		switch (element['type']) {
			...
			case COLOR:
				...
				output 		+= '<input type="text" class="text-field themedo-color-field" value="' + nl2br(element['value'],false) + '" id="' + element['id'] + '" name="' + element['id'] + '" size="50"/>';
			break;
			
			case GALLERY:
				output 		+= '<a href="'+element['id']+'" class="themedo-gallery-button themedo-shortcodes-button">Attach Images to Gallery</a>';
			break;
			...
			case UPLOAD:
				...
				output 		+= '<input type="hidden" class="themedo-form-text themedo-form-upload themedo-input" name="'+element['id'] +'" id="'+element['id'] +'" value="'+element['value'] +'" />' + "\n";
				output 		+= '<a href="'+element['id'] +'" class="themedob-upload-button '+button_class+'" data-upid="'+element['upid'] +'">'+button_text+'</a>';
				output		+= '<a href="'+element['id'] +'" style="'+edit_style+'" class="themedob-edit-button" data-upid="'+element['upid'] +'">Edit</a>';
				output 		+= '</div>';
