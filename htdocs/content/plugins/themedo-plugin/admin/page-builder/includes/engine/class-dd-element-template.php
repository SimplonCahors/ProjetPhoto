...
abstract class DDElementTemplate {
				...
				for ($i = 0; $i < $elementsCount; $i++) {
					...
					foreach ($element['elements'] as $dynamic_element ) {
						...
						$output 	.= "<div class='clearfix form-element-container 
										themedo-element-child'><div class='name-description'>";
					...
					$output 	.="<a class='child-clone-row-remove themedo-shortcodes-button' 
									href='JavaScript:void(0)'>".__('Remove', 'themedo-core')."</a>";
					$output 	.= "</td></tr>";
				}
				$output 		.= "<tr><td><a id='themedo-child-add' href='JavaScript:void(0)'>".$element['buttonText']."</a>
									</td></tr>";
				$output 		.= "</table>";
	...
	private function parseElementType( $element ) {
		...
		switch ($element['type']) {
			...
			case ElementTypeEnum::COLOR:
				$output .= '<input type="text" class="text-field themedo-color-field" value="'.nl2br($element['value']).'" id="'.$element['id'].'" name="'.$element['id'].'" size="50"/>';
			break;
			
			case ElementTypeEnum::GALLERY:
				$output .= '<a href="'.$element['id'].'" class="themedo-gallery-button themedo-shortcodes-button">'.__('Attach Images to Gallery', 'themedo-core').'</a>';
			break;
			
			case ElementTypeEnum::UPLOAD:
				$button_class = ( $element['value'] == 'themedo-hidden-img' ? '' 		: 'remove-image' ); 
				$button_text  = ( $element['value'] == 'themedo-hidden-img' ? __('Upload', 'themedo-core') 	: __('Remove', 'themedo-core') ); 
				$output .= '<div class="themedo-upload-container">';
				$output .= '<img src="' . $element['value'] . '" alt="Image" class="uploaded-image" />';
				$output .= '<input type="hidden" class="themedo-form-text themedo-form-upload themedo-input" 
							name="' . $element['id'] . '" id="' . $element['id'] . '" value="' . $element['value'] . '" />' . "\n";
				$output .= '<a href="' . $element['id'] . '" class="themedo-upload-button '.$button_class.'" 
			...
			case ElementTypeEnum::ICON_BOX:
				$iconsArray = themedoHelper::GET_ICONS_LIST();
	...
	*/
	public function create_visual_editor( $params ) {
		...
		$innerHtml.= "<div class='themedo-element-label'>".$this->config['name']."</div>";
