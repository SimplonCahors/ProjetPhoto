<?php
/**
 * themedoHelper class with static methods
 */
class themedoHelper  {
	/**
	...
	public static function OBJECT_TO_ARRAY( $object ) {
		...
		if ( is_array($object) ) {
			...
			*/
			return array_map("themedoHelper::OBJECT_TO_ARRAY", $object);
		}
		else {
				// Return array
				return $object;
	 ...
	 * @return: data array
	 */
	public static function avalon_td_shortcodes_categories ( $taxonomy, $empty_choice = false, $empty_choice_label = 'Default' ) {
	 ...
	 * @return: data array
	 */
	 public static function avalon_td_create_dropdown_data ( $start, $end, $dataArray = array() ) {
		 
		 for ($i = $start; $i <= $end; $i++ ) {
			 $array['avalon_td_' . $i] = $i;
		 }
	...
	public static function GET_ICONS_LIST() {
		...
		$icons = new FAIterator(avalon_td_BUILDER_FA_PATH);
		$iconssArray = array();
		$output = '';
	...
	public static function get_layerslider_slides () {
		global $wpdb;
		$slides_array['avalon_td_0'] 	= 'Select a slider';
		// Table name
		...
		if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
			...
			if(isset($slides) && $slides){
				foreach($slides as $key => $val){
					$slides_array['avalon_td_'.$key] = 'LayerSlider #'.($key);
				}
			}
		}
	...
	public static function get_revslider_slides () {
		...
		global $wpdb;
		$revsliders['avalon_td_0'] 		= 'Select a slider';
			...
			if(function_exists('rev_slider_shortcode')) {
				...
				if($get_sliders) {
					foreach($get_sliders as $slider) {
						$revsliders['avalon_td_'.$slider->alias] = $slider->title;
					}
				}
			}
		}
	...
	public static function get_elasticslider_slides () {
		...
		$slides 			= get_terms('avalon_td_es_groups');
	...
	/**
	 * Returns array of theme themedo slider slide groups
	 * @param NULL
	 * @return array slide keys array
	 */
	 ...
	 * @return array
	 */
	public static function get_reversed_choice_data_2 () {
		$reverse_choices = array( 
									'on' => 'On', 
									'off' => 'Off' 
								);
